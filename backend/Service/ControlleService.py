...
class OperationsControlCenter:
    ...
    def generate_tasks(self, file_path):
        ...
        try:
            ...
            for row in range(2, ws.max_row + 1):
                ...
                # 读取各个字段
                is_emergency = (ws.cell(row=row, column=2).value.strip() == '是') \
                    if ws.cell(row=row, column=2).value else False
                ...
                task_type = ws.cell(row=row, column=3).value.strip() if ws.cell(row=row, column=3).value else " "
                ...
                sensor_type = ws.cell(row=row, column=4).value.strip() if ws.cell(row=row, column=4).value else None
                resolution = float(ws.cell(row=row, column=5).value) if ws.cell(row=row, column=5).value else None
                cluster_name = ws.cell(row=row, column=8).value.strip() if ws.cell(row=row, column=8).value else None
                cycle_time = 60 * int(ws.cell(row=row, column=9).value) if ws.cell(row=row, column=9).value else 3600
                appoint_time = ws.cell(row=row, column=10).value.strip() if ws.cell(row=row,
                                                                                    column=10).value else None
                appoint_time = datetime.strptime(appoint_time, '%Y,%m,%d,%H,%M,%S') if appoint_time else None
                ...
                # 解析时间范围
                time_range = ws.cell(row=row, column=6).value.strip() if ws.cell(row=row, column=6).value else None
                if time_range:
                    time_range = time_range.split('-')
                    start_time = datetime.strptime(time_range[0].strip('()'), '%Y,%m,%d,%H,%M,%S')
                    end_time = datetime.strptime(time_range[1].strip('()'), '%Y,%m,%d,%H,%M,%S')
                    start_time1 = start_time.replace(tzinfo=timezone.utc)
                    end_time1 = end_time.replace(tzinfo=timezone.utc)
                else:  # 如果没有指定时间范围，则默认为当前时间到一天后
    ...
    def generate_single_task(self, data):
        ...
        if data['appoint_time'] == "" or data['appoint_time'] is None:
            ...
            appoint_time = datetime.strptime(data['appoint_time'].strip(), '%Y-%m-%d %H:%M:%S') if not (
    ...
    def planning_tasks(self, tasks, model):
        ...
        with ((self.app.app_context())):
            ...
            while i < len(tasks):
                ...
                if task1.parent_task_id or task1.friend_task:
                    if task1.parent_task_id:
                        processed_tasks.append(task1)
                        main_set.add(self.tasks_buffer[task1.parent_task_id])
                    i += 1
                    continue
                else:
                    j = i + 1
            ...
            # 如果有规划结果，则直接添加统计数据
            if schedule:
                used = str(schedule[model]["cluster_status"])
                # 打开文件，使用追加模式 ('a')。若文件不存在，则会创建该文件。
                with open('example.txt', 'a', encoding='utf-8') as file:
                    # 向文件中追加一行新内容。注意这里手动添加了换行符 '\n'
                    file.write(f"{used}\n")
                ...
                for cluster in self.satellite_network.clusters:
                    if cluster.name not in schedule[model]["cluster_status"]:
                        ...
                        schedule[model]["cluster_status"][cluster.name]["time_occupancy_ratio"] = \
                            schedule[model]["cluster_status"][cluster.name]["cluster_time_cost"] / (
                                    diff_time * cluster.satellite_count)
                data = {
    ...
    def run(self):
        ...
        library_dir = os.path.join(os.getcwd(), 'library')
        files_to_delete = [
            os.path.join(library_dir, sat_parms),
            os.path.join(library_dir, TLE),
            os.path.join(library_dir, cluster_parms)
        ]
        ...
        with app.app_context() if app else nullcontext():
            ...
            # 创建卫星网络
            self.satellite_network = SatelliteNetwork(self.network_config, self.sats_config, self, self.is_random, app)
            # 启动卫星网络线程
            # 初始化所有的卫星轨迹
            # 初始化所有卫星的轨迹
            print("初始化所有卫星的轨迹")
        ...
        while True:
            ...
            # 同步时间和倍速
            # 如果没有新任务，则倍速恢复到1
            if not self.satellite_network.new_tasks:
                self.satellite_network.time_multiple = 1
            # 同步时间
